...
- **JCR/EI indexing annotation** (in `.journal-info` line, as colored badges):
  ...
  - **EI status is its own lookup:** EI-Compendex indexing is independent of SCIE, and must be checked separately. letpub's "Indexing" row often omits EI even for EI-indexed journals (e.g. Optik, ISSN 0030-4026, is EI but letpub's Indexing row only lists SCI collection + Scopus) — so an absent EI row does NOT mean the journal is non-EI. The authoritative source is OneScholar/scigreat (field `ei: "EI"`). Do NOT annotate a journal as 非 EI from a letpub omission; if you cannot confirm EI status, simply omit the EI badge rather than asserting its absence.
